...
import { AdminCourseType } from "@/app/dashboard/data/admin/admin-get-course";
...
export function CourseStructure({ data }: iAppProps) {
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (activeType === "chapter") {
      ...
      if (!targetChapterId) {
        toast.error("Couldn't determine the chapter of reordering");
        return;
      }
      ...
      if (oldIndex === -1 || newIndex === -1) {
        toast.error("Couldn't find chapter old or new index for reordering");
        return;
      }
    ...
    if (activeType === "lesson" && overType === "lesson") {
      ...
      const updateLessonForState = reorderedLessons.map((lesson, index) => ({
        ...lesson,
        order: index + 1,
      }));
                  ...
                  <Card>
                    ...
                    >
                      ...
                      <CollapsibleContent>
                        <div className="p-1">
                          <SortableContext
                            items={item.lessons.map((item) => item.id)}
                                ...
                                {(lessonListeners) => (
                                  <div className="flex items-center justify-between p-2 hover:bg-accent rounded-sm">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <Link
                                        href={`/dashboard/admin/courses/${data.id}/${item.id}/${lesson.id}`}
                                      >
                                        {lesson.title}
                                      </Link>
                                    </div>
